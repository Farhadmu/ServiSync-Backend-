...
- **Project**: ServiSync
- **Type**: Enterprise Field Service Management System (B7A6 Assignment — Field Service Management 🔧)
...
```
Project Name    : ServiSync — Field Service Management Platform
Backend Repo    : https://github.com/Farhadmu/ServiSync-Backend-
Live API        : https://servisync-backend.onrender.com
API Docs        : https://documenter.getpostman.com/view/56403424/2sBYAvwqwU
Admin Email     : admin@servisync.com
...
|---|:---:|---|
| API Documentation (Postman) | ✅ | [Published Postman Docs](https://documenter.getpostman.com/view/56403424/2sBYAvwqwU) — `postman/` folder in repo |
| Consistent `{ success, message, data }` / `{ success, message, errors }` responses | ✅ | `src/utils/response.ts`, `src/middlewares/errorHandler.ts` |
| 20+ meaningful commits | ✅ | Git history (`feat:`, `fix:`, `docs:` prefixed) |
| Input validation (Zod) on all applicable endpoints | ✅ | Per-module `*.validation.ts` files + `src/middlewares/validateRequest.ts` |
| Auth: Email/Password + Google OAuth | ✅ | `src/modules/auth/` |
| Role-based authorization (3+ roles: Customer, Technician, Manager, Admin) | ✅ | `src/middlewares/authenticate.ts`, per-route `authorize(...)` guards |
| Working demo Admin credentials | ✅ | See Submission Summary + Section 9 below |
| Real payment integration (Stripe) | ✅ | `src/modules/payments/` — Checkout Session + webhook verification |
| PostgreSQL + Prisma with relations, constraints, indexing, transactions | ✅ | `prisma/schema.prisma`, `prisma/migrations/` |
| Live deployed API | ✅ | https://servisync-backend.onrender.com |
| API versioning (`/api/v1/...`) | ✅ | All routes mounted under `/api/v1` |
| Pagination on list endpoints | ✅ | `src/utils/pagination.ts` (e.g. `GET /service-requests?page=&limit=`) |
| Filtering / sorting / search | ✅ | Technicians (skill filter), Service Requests (status filter), Admin Users (search) |
| Soft deletes (`deletedAt`) | ✅ | Service Categories, Service Types, Users |
| Audit logs / activity tracking | ✅ | `src/utils/auditLog.ts`, `GET /api/v1/admin/audit-logs` |
| Rate limiting | ✅ | `src/middlewares/rateLimiter.ts` (Auth / General / Upload / Payment tiers) |
| Security headers & CORS | ✅ | `helmet`, `src/config/cors.ts` |
| Business logic beyond CRUD | ✅ | Skill-matched assignment, double-booking prevention, status-transition workflow, decimal invoice calculation |
...
The complete, published Postman documentation (all folders — Auth, Service Categories, Technicians, Service Requests, Assignments & Scheduling, Work Orders & Service Execution, Invoices & Billing, Payments, Feedback, Notifications, Admin & Analytics, and RBAC/403 test cases) is available here:

👉 **[https://documenter.getpostman.com/view/56403424/2sBYAvwqwU](https://documenter.getpostman.com/view/56403424/2sBYAvwqwU)**

The raw collection and environment JSON files are also included in this repo under `postman/`.